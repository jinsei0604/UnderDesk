...
- **掘削アニメ（2026-07-12更新）**: minion_0 は6コマ（idle + 振り上げ + 頂点 + 打ち込み開始 + ピーク/砂煙 + 収まる、tools/extract_sprites.gd の "miner_v2" プリセット、原本 assets/reference/miner_sheet_v2.png）。ベースのidle絵（minion_0.png）は元の素材のまま維持——新シートの該当セルは隣接ポーズが常ににじみ込み、自動グリッド抽出では綺麗に切り出せなかったため
  - **ハマった罠1（向き不一致）**: 新シートの生コマは全部idleと逆向きだった。`FLIP_X_OUTPUTS`に追加するキーを忘れると、掘削中だけ体が反転して見える致命的なバグになる。**minion_0系の新規フレームを追加するときは必ずFLIP_X_OUTPUTSに列挙し、idleと並べて向きを目視比較すること**
...
## 実装済みの主要システム（2026-07-12セッションで追加）
...
- **祭壇のお供え**: `sim.offer_at_altar(item_id)`。コイン費用は UD.ALTAR_OFFER_BASE_COST×1.4^Lv、+1掘削力/Lv、Lv5からは UD.ALTAR_ITEM_RANK_TIERS のランクの宝物1個も消費。祭壇建設が前提。埋め戻しでリセット（run-scoped）
- **ギルド交換**: 酒場はギルドに改名（room idは`tavern`のまま、locale名のみ変更）。`sim.exchange_item(target, consume)` で下位ランク消費→上位1個（レートは UD.ITEM_EXCHANGE_COSTS: Z←S×3/S←A×5/A←B×7/B←C×10、C・Dは宝箱限定）。**対人交換はSteam連携（Steam Inventory Service）で同じコマンドに載せる設計**。UIの消費内訳は所持数の多い順に自動選択
...
- **調査書カード機能は削除済み（2026-07-12、ユーザー判断）**: Steamページ公開前は見せる相手がいないため撤去。P4（デモ&露出）のマーケティング施策で必要になったら `git log` から `src/daily/survey_card.gd` / `src/window/clipboard.gd` を復元すること。デイリー異変システム自体（`UDDaily`・`data/anomalies/`）は現役
...
- **掘削アニメ**: 3コマループは「掘削専用」（歩行中は静止フレーム）。歩行中に再生すると「ぎこちない」動きに見える
- **ダイアログのイラスト化**: ショップ/宝物/書庫/事件簿のItemListは`icon_mode=ICON_MODE_LEFT`＋アイコン付き。`UDArtLibrary.icon_or_placeholder(key, seed, shape)`が実アート（`item_<id>.png`/`shop_<id>.png`/`prestige_<id>.png`/`<doc_id>.png`）があればそれを使い、無ければ`placeholder_icon`で手続き生成（gem/rune/book形状、seedからHSVで色決定）。実アートを置くだけで自動的に差し替わる
