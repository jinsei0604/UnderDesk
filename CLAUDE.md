...
- **ダイアログの背景イラスト（2026-07-13〜、ユーザーの自作ドット絵を6画面に順次組み込む予定の1本目）**: `assets/art/dialog_bg_<archive|treasure|shop|altar|guild|dorm>.png` を置くと、対応する`UDCardDialog`のカードグリッド背後に敷かれる。**背景はダイアログ全体を覆う**（`_background_rect`は`root`直下・全面、`STRETCH_KEEP_ASPECT_COVERED`）。6画面すべて`enable_art_chrome(title, close)`で**枠なし＋中央タイトル＋右上「✕ 閉じる」ボタン**のアート主導UIに統一済み（ショップはタイトル/閉じる/コインが画像に焼き込み＋看板ホットスポット、他5枚はコードでクロームを重ねる）。`main.gd`の各`_build_xxx_dialog()`で`set_background_frames(_dialog_bg_frames("dialog_bg_xxx"))`を渡すだけ。未配置なら非表示（従来通りの暗いキャビネット地）。合成済みのモックアップ画像しか届かない場合は、Godotの`Image.get_region()`でカード領域・背景領域を切り出し、アイコン部分は`tools/extract_sprites.gd`と同じフラッドフィル透明化＋トリム＋正方形化を再利用する
  ...
  - **ダイアログの額縁オーバーレイ（2026-07-16実装、同日中に9-slice→均一拡大に方式転換）**: `assets/art/dialog_frame.png`（ユーザー提供の1536x1024イラストを黒背景flood-fill透明化、原寸のまま）を`UDCardDialog.set_frame_overlay(tex)`で書庫/宝物/祭壇/宿舎の4画面に適用。**実装方式**: 最初はGodotのNinePatchRect（角のドット絵を保ったまま辺だけ伸縮する9-slice）で試したが、額縁自体の縦横比(1536:1024)とダイアログの縦横比が違うため中央の帯（星の意匠など）が不自然に伸び、ユーザーから「形がおかしい」と指摘された。**`_background_rect`と同じ`STRETCH_KEEP_ASPECT_COVERED`（縦横比を保ったまま拡大しはみ出た分を均等に切り落とす）に変更**して解決——伸縮ではなく拡大なので額縁自身の形は歪まない。トレードオフとして枠の太さがダイアログサイズに比例して変わる（9-sliceの固定ピクセル角と違う）ため、`_frame_overlay`（`TextureRect`、`content`直下の最後の子＝最前面）の`resized`シグナルに`_layout_frame_margin()`を繋ぎ、実際の表示スケールから枠の太さを毎回計算し直して`_content_margin`(MarginContainer、`column`を包む)の余白に反映する。**中身は枠の内側に収める**: 素のダイアログの内側面積(960x560=`BASE_MIN_SIZE`)を保てるよう、`set_frame_overlay()`が額縁画像の縦横比のまま`min_size`を算出し直す（960x560→約1364x910、額縁が太いぶんだけダイアログ自体を一回り大きくする設計）。背景アート(`_background_rect`)は額縁の下に潜り込む形でダイアログ全体を覆ったまま（額縁が写真の縁を隠すのと同じ発想）。**ショップとギルドは意図的に対象外**（2026-07-16、実装後にプレビュー画像で確認して判断）: どちらも看板ボタン/コインバッジ/閉じるプラーク/「アイテム交換」等の文字が画像の端近くまで焼き込まれており、額縁の太い枠（ダイアログ幅の約12%）がそれらを物理的に覆い隠す（ギルドは「アイテム交換」の最後の文字が欠けて表示された）。既存背景画像は変更しない方針のため画像側を調整できず、汎用アート＋コード描画UIで端に固定コンテンツが無い書庫/宝物/祭壇/宿舎の4画面だけに適用
