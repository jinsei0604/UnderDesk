...
- **戦闘ビューの背景（2026-07-19〜）**: 放置中=`assets/art/battle_background.png`（序章キャンプ地）、ボス戦中=`assets/art/boss_background.png`（ポータル遺跡）。どちらも1枚絵をカバーフィット（アスペクト維持・はみ出しクロップ）で表示、タイル・スクロールなし。`_fN`アニメ規約は使えるが現在は静止画。パーティは`PARTY_FORMATION`（main.gd、ユーザーの配置リファレンス`ボス戦仲間配置.png`準拠の前後2列フォーメーション、奥から手前の順に描画）で左側に配置、ボスは右に`BOSS_ICON_PX`=176pxで大きく表示。常駐ストリップは旧来のコンパクト行のまま（レイアウト再設計予定のため意図的に未変更）
